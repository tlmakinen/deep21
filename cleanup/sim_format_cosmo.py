...
if __name__ == '__main__':
	...
	## "global" string with name to disk directory where simulations are stored
	...
	for SNUM in np.arange(1,NUM_SIMS + 1):
		# Open the Fits files & take every 30th freq bin
		fgd = np.array([fits.getdata("%s/run_%s_s1%03d/%s_%03d.fits"%(dirstr,type_str,SNUM,type_str2,(nu*N_SKIP+NU_START)),1) for nu in range(N_NU)],dtype=np.float64).T
		...
		# get the indices of the pixels which actually are in the larger pixel
			...
			print('to_rearr: ', to_rearr.shape)
			to_rearr = (to_rearr[np.argsort(to_rearr_inds)])[rearr]
			to_rearr = np.reshape(to_rearr,(WINDOW_LENGTH,WINDOW_LENGTH,N_NU)) # EDIT: from NU_AVG -> N_NU
