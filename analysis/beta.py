...
output_base = "/mnt/home/tmakinen/ceph/deep21_results/unpolarized/"
outdirs = [output_base + m for m in ['beta_minus/', 'beta_plus/']]
...
if __name__ == '__main__':
    ...
    for l,fg_type in enumerate(['beta_minus','beta_plus']):
        ...
        print('working on simulation %s, writing to %s'%(fg_type, outdir))

        pca3 = np.load(data_path + 'pca3.npy')
        pca6 = np.load(data_path + 'pca6.npy')
        cosmo = np.load(data_path + 'cosmo.npy')
        noise = np.load(data_path + 'cosmo_noisy.npy')
